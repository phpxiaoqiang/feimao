...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    this.fileProgressWrapper.find('.status').text("宸蹭笂浼�: " + size + " 涓婁紶閫熷害锛� " + formatSpeed + "/s");
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    str = "<div><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
        ...
        "<div class=process-status><strong>杞爜鐘舵€�:</strong><a href=" + persistentUrl + " target='_blank' > " + '绛夊緟澶勭悊' + "</a></div>";
    td.html(str).removeClass().next().next('.status').hide();

    var progressNameTd = this.fileProgressWrapper.find('.progressName');

    var Wrapper = $('<div class="Wrapper"/>');
    var playBtn = $('<input class="origin-video btn  btn-primary play-btn" type="button" value="鎾斁鍘熻棰�">');
    playBtn.attr('data-url', url);
    var processedPlayBtn = $('<input class=" btn  btn-info play-btn" type="button"  style="display:none" value="鎾斁杞爜鍚庤棰�">');

    Wrapper.append(playBtn);
    Wrapper.append(processedPlayBtn);
    progressNameTd.append(Wrapper);
    $('table button.btn').hide().parents('tr').next().hide(); //闅愯棌杩涘害鎸夐挳
        ...
        $.ajax({
            ...
            switch (item.code) {
                case 0:
                    ...
                    return;
                case 1:
                    statusAnchor.text('绛夊緟澶勭悊');
                    break;
                case 2:
                    statusAnchor.text('姝ｅ湪澶勭悊');
                    break;
                case 3:
                    statusAnchor.text('澶勭悊澶辫触');
                    clearInterval(timerId);
                    break;
                case 4:
                    statusAnchor.text('閫氱煡澶辫触');
                    clearInterval(timerId);
                    break;
            }
        });
