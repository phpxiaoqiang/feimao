...
/**
 * Wxuser is the class for the `user` application component that manages the user authentication status.
 *
 ...
 *
 * Note that Wxuser only maintains the user authentication status. It does NOT handle how to authenticate
 ...
 *
 * Wxuser is configured as an application component in [[\yii\web\Application]] by default.
 ...
 * ```php
 * 'user' => [
 *     'identityClass' => 'app\models\Wxuser', // Wxuser must implement the IdentityInterface
...
class User extends Component
{
    ...
    public function init()
    {
        ...
        if ($this->identityClass === null) {
            throw new InvalidConfigException('Wxuser::identityClass must be set.');
        }
        if ($this->enableAutoLogin && !isset($this->identityCookie['name'])) {
            throw new InvalidConfigException('Wxuser::identityCookie must contain the "name" element.');
        }
    }
    ...
    public function login(IdentityInterface $identity, $duration = 0)
    {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            if ($this->enableSession) {
                $log = "Wxuser '$id' logged in from $ip with duration $duration.";
            } else {
                $log = "Wxuser '$id' logged in from $ip. Session not enabled.";
            }
    ...
    protected function loginByCookie()
    {
        ...
        if (isset($data['identity'], $data['duration'])) {
            ...
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $id = $identity->getId();
                $ip = Yii::$app->getRequest()->getUserIP();
                Yii::info("Wxuser '$id' logged in from $ip via cookie.", __METHOD__);
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Yii::$app->getRequest()->getUserIP();
            Yii::info("Wxuser '$id' logged out from $ip.", __METHOD__);
